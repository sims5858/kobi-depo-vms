...
const Dashboard = () => {
  ...
  const [sortOrder, setSortOrder] = useState('desc'); // 'desc' = en yüksekten en düşüğe (varsayılan)
  ...
  useEffect(() => {
    ...
    // Zamanı client-side'da güncelle
    const updateTime = () => {
      const now = new Date();
      setCurrentTime({
  ...
  const loadDashboardData = async () => {
    try {
      ...
      console.log('Koli verileri yüklendi:', koliData.length);
      
      // Ürün listesini yükle
      const urunResponse = await fetch('/api/urun');
      const urunData = await urunResponse.json();
      console.log('Ürün verileri yüklendi:', urunData.length);
      
      // Aktivite verilerini yükle
      const activitiesResponse = await fetch('/api/dashboard/activities');
      const activitiesData = await activitiesResponse.json();
      console.log('Aktivite verileri yüklendi:', activitiesData.length);
      ...
      const toplamUrun = urunData.length; // Gerçek ürün sayısı
      const toplamAdet = urunData.reduce((sum, urun) => sum + (urun.stok_miktari || 0), 0); // Toplam stok miktarı
      ...
      // Aktivite verilerini set et (zaten yukarıda yüklendi)
    ...
    <div className="page-transition" style={{ minHeight: '100vh', backgroundColor: '#f8f9fa' }}>
        ...
        <div className="d-flex align-items-center justify-content-between">
          ...
          <div className="d-flex align-items-center">
            <div className="text-end me-3">
              <p className="small fw-medium text-dark mb-0">{currentTime.date || 'Yükleniyor...'}</p>
              <p className="text-muted" style={{ fontSize: '11px' }}>{currentTime.time || 'Yükleniyor...'}</p>
            </div>
            <Button 
      ...
      <div className="p-4">
        ...
        <Row className="mb-4">
          ...
          <Col lg={3} md={6} className="mb-3">
            <div className="anim-fade-in delay-4">
              <StatCard
                title="Toplam Adet"
                value={stats.toplamAdet}
                icon={BiPulse}
                color="secondary"
                trend={4.2}
                subtitle="Tüm stok miktarı"
              />
            </div>
          </Col>
        </Row>
              ...
              <Card.Header className="bg-white border-bottom" style={{ borderColor: '#e9ecef', borderRadius: '12px 12px 0 0' }}>
                <div className="d-flex align-items-center justify-content-between">
                  ...
                  <Button
                    ...
                    className="text-xs"
                  >
                    {sortOrder === 'desc' ? '↓ Yüksek → Düşük' : '↑ Düşük → Yüksek'}
                  </Button>
                </div>
              </Card.Header>
              <Card.Body className="p-4">
                  ...
                  {getSortedItems().length > 0 ? (
                    getSortedItems().map((item, index) => (
                      <div key={index} className="d-flex align-items-center justify-content-between p-3 mb-3 bg-light rounded" style={{ borderRadius: '8px' }}>
                        <div className="d-flex align-items-center">
                          <div className="bg-primary bg-opacity-10 rounded d-flex align-items-center justify-content-center me-3" style={{ 
                            minWidth: '60px', 
                            height: '48px',
                            padding: '0 8px'
                          }}>
                            <span className="small fw-semibold text-primary" style={{ 
                              fontSize: '10px',
                              wordBreak: 'break-all',
                              textAlign: 'center'
                            }}>{item.koli_no}</span>
                          ...
                          <div className="flex-grow-1">
                            <p className="mb-0 small fw-medium text-dark">Koli #{item.koli_no}</p>
                            <p className="mb-0 text-muted" style={{ fontSize: '11px' }}>
                              {item.urun_sayisi} ürün • {item.toplam_adet} adet
                            </p>
                          </div>
                        </div>
                        <div className="d-flex align-items-center">
