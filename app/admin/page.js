...
const AdminPanel = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const url = '/api/admin/kullanici';
      const method = editingUser ? 'PUT' : 'POST';
      
      // PUT isteği için ID'yi ekle
      ...
      if (response.ok && data.success) {
        ...
        toast.error(data.error || 'Kullanıcı kaydedilirken hata oluştu');
      }
    } catch (error) {
  ...
  const handleDelete = async (userId) => {
    if (typeof window !== 'undefined' && !window.confirm('Bu kullanıcıyı silmek istediğinizden emin misiniz?')) {
      return;
    }
    ...
    try {
      ...
      const response = await fetch(`/api/admin/kullanici?id=${userId}`, {
        method: 'DELETE',
      });
      ...
      if (response.ok) {
        ...
        toast.error(`Kullanıcı silinirken hata oluştu: ${errorData.error || 'Bilinmeyen hata'}`);
      }
    } catch (error) {
    ...
    <div className="page-transition">
      <div className="d-flex justify-content-between align-items-center mb-4">
        <div className="d-flex align-items-center">
          ...
          <div>
            <h1 className="h3 mb-0 text-primary">Admin Panel</h1>
